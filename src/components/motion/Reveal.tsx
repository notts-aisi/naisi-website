...
type Props = {
  ...
  staggerMs?: number;
  /** Cap on distinct stagger slots. Default 8. */
  staggerMax?: number;
  ...
  const renderedChildren = staggerChildren
    ? Children.map(children, (child, i) => {
        ...
        return cloneElement(child as React.ReactElement<{ style?: CSSProperties }>, {
          style: {
            ...(childProps.style ?? {}),
            // Clamped so long lists arrive as a group past the cap instead
            // of tailing off for seconds.
            "--stagger-index": Math.min(i, staggerMax - 1),
          } as CSSProperties,
        });
      })
    : children;
