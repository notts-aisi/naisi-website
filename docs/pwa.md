...
What sends pushes: the self-test, the TASK pipeline, and the two COURSE DECISION moments. Every task email (added-to-task, comment, mention, review request, review outcome) is mirrored to the recipient's enabled devices by `src/lib/push/taskNotifications.ts`. An admissions decide (both the appointment and the decline branch) and an allocation publish are mirrored by `src/lib/push/courseNotifications.ts`. The policy is mirror-the-email exactly: the config/task-emails kill switch covers task push for free because every route early-returns before its sends, and volume is bounded by the existing email budget.

Enabling notifications on a device is still the opt-in that makes any of this happen. On top of it sit two account-level switches, `profile.notifications.push.tasks` and `.courseDecisions`, edited on their own card on /profile and read server-side by `src/lib/push/preferences.ts`. They are a SIBLING of `channels` and `categories` in `src/lib/firestore/notifications.ts`, never a member of either, because `channels` means email-address routing and a transport with no address folded into it would break `addressesForSend`. Both default true: an absent key is "hasn't answered", and defaulting false would have switched off, for everyone at once, notifications people had already enabled. A failed preference read fails closed.

A decision push names the round, the course or the group and never the reason: a lock screen is a surface the member did not choose. The weekly course nudge, the admissions deadline reminders and the stage-release announcement deliberately do NOT push, and `tests/push-preferences.test.mjs` asserts none of those three files imports anything from `lib/push`. The stage release is an OPEN OWNER DECISION: the V3 contract says it pushes under `courseDecisions`, the guard says it never pushes, and the guard stands until somebody settles it because "never" is the reversible answer. Event reminders are still email-only too.
