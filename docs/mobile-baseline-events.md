...
- **The RSVP bot gate (9 September 2026).** `POST /api/events/[id]/rsvp` was an unauthenticated, unthrottled, un-CAPTCHA'd mailer with a caller-chosen recipient, and it is now gated the way `/api/register` is. The consequence for this flow is visual: `RsvpForm` mounts `RecaptchaInvisible`, so Google's floating badge now sits on the public event page. It is `position: fixed` in the bottom-right corner and roughly 70 by 60 CSS pixels, which is the corner the sticky RSVP panel and the submit button share below `--bp-md`. No module in the frozen list changed, and no CSS did, so nothing here is expected to move; what is owed is an eye on whether the badge covers the submit button or the food callout at 375 and 414, and in landscape at 844 by 390 where it competes with the notch inset. The same badge has been on `/register` since sign-in was reworked, so the pattern is not new to the site, only to this flow.
