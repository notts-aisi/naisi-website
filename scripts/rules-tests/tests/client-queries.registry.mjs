...
export const REGISTRY = [
  ...
  },
  {
    ...
    reason:
      "The member roster, on `role in ['member','committee','admin']` (plus 'rejected' when the caller ticks it). Two shapes for one clause; both are lists over `users`, which is admin and SU-committee only, so the outcome is the same either way. FINDING, recorded rather than hidden: RunEditor.tsx mounts this hook unconditionally under /admin/courses, whose gate admits a `draftCourse` or `approveCourse` holder who is a plain member, and for that caller the roster read is refused. RoundEditor got this right by mounting the hook only inside its admin-only child.",
    outcomes: {
      "signed-out": "refused",
      pending: "refused",
      member: "refused",
