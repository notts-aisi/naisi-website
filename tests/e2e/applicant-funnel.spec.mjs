  ...
  try {
    ...
    await step("the availability grid paints and the marks persist", async () => {
      ...
      const probe = await page.evaluate(async () => {
        ...
        // BOTH ends right, TWICE RUNNING. Holding once is not the same as
        // being settled: `waitForHydration` returns as soon as a
        // `__reactFiber/**
 * The applicant funnel, end to end, in a real browser.
 *
 * Public course page -> the signed-out gate on /apply/[roundId] -> sign in ->
...
test("applicant funnel: apply, withdraw, re-apply, enrol, drop out", { skip: skipReason ?? false }, async (t) => {
 ...
 key exists on the node, which is the first instant
        // React touched it, not the instant the page stopped moving. On
        // 8 September 2026 a Linux run measured a good position, and by the
        // time the pointer went down the layout had shifted out from under it:
        // the grid received 0 pointerdown, 0 pointermove, 0 pointerup and
      ...
      // and then stop being true while the harness makes its next round trip,
      // and a drag that starts off the grid paints nothing and blames the
      // component.
