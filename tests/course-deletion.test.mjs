...
test("GUARD — the survivors are not counted as deaths, and the arithmetic follows the fates", () => {
  ...
  // Seventeen counters at ten rows each: twelve run counters that die,
  // `materialNotes`, `auditRows` and `schedulerMarkers` among them, the live
  // runs counter, and the four survivors (`admissionSeatOffers` is released,
  // not destroyed).
