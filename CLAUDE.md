...
trackedLinks/{slug}     One short link, naisi.uk/q/<slug>: { slug, label,
                          destination (a path on this site, or an https
                          address), type ("qr" | "link"), campaign (a label,
                          not a document), active, countOffsite, createdAt,
                          updatedAt, createdByUid, updatedByUid }. The slug is
                          the document id and is PRINTED, so it never changes.
                          Admin read and write, client-direct; NO DELETE for
                          anybody, a link is switched off with `active`.

linksPage/main          What the public /links page says: { applications:
                          { fellowship|facilitator|incubator: { open, href } },
                          groups[]: { id, heading, rows[]: { id, label, sub,
                          href, soon, hidden } }, updatedAt, updatedByUid }.
                          ADMIN-ONLY in both directions although the page is
                          public: the page reads it on the server and projects
                          out hidden rows and the editor's uid.
...
linkScanDays/{id}       Scan counters for printed QR codes. doc-id:
                        {slug}__{YYYY-MM-DD}, one per code per LONDON day:
                          { slug, date, count, hours: { "00".."23": n } }
                        Nothing about who scanned. Incremented by
                        POST /api/q/[slug]/scan. Admins read it (the
                        /admin/links dashboard); NOBODY writes it, admins
                        included.
...
- `(app)/admin/(admin-only)/**` calls `requireAdminPage()`. Everything that is
  ...
  Registrations, Projects, Newsletter, Links, Sources, Subscriptions, Email designs,
...
- **Public site**: landing page (hero, intro, stats, one about sentence, subscribe form; on 21 Sep 2026 the owner had everything below that sentence COMMENTED OUT in `src/app/(public)/page.tsx`, kept and not deleted: the engagement tiers, upcoming events, reading lists, Instagram carousel, Elsewhere cards and committee preview. The subscribe form stays because the hero's second button, four links in the courses funnel and the Instagram bio redirect `/stay-in-touch` all land on `#stay-in-touch`), Members directory, Resources (HIDDEN FOR NOW: a temporary redirect to `/` in `next.config.ts` takes over from the untouched page, and the three links to it are commented out; `tests/front-facing-copy.test.mjs` says how to bring it back, and also holds that no copy promises how often the society emails), News list + article (SSR + OG tags), public Events list + event detail page with the full RSVP flow (submit, confirmation, change request, self-cancel).
...
- **Admin dashboard tabs**: Approvals, Members (role / title / bio / `suRecognised` / `permissions` / `tracks` edit + full profile edit + hard delete + "View as" debug impersonation), Projects (CRUD + archive), Newsletter, Links (short links and QR codes: create, repoint, campaign, switch off), Sources, Subscriptions, Email designs (application email templates), Deliverability (send log + suppression list), Task templates, Danger zone.
...
- **Short links, `/links` and scan counting**: every printed QR code encodes `naisi.uk/q/<slug>`, never the destination, so where a code goes can change after the print run exists. The slugs in `src/lib/campaign/printedLinks.ts` are ON PAPER and permanent: never removed, renamed or reused. The scan is answered by `src/app/api/q/[slug]/route.ts` behind a rewrite, from a `trackedLinks` record an admin repoints at `/admin/links`: 307 and `no-store`, never 308, which a phone caches for good. It falls back to the last record that server saw, then to the printed list, then to `/links`, so a printed code works with no record and with Firestore down. The `Location` is RELATIVE and built by hand, because behind the hosting proxy `req.url` carries an internal host and `NextResponse.redirect(new URL(path, req.url))` would send every printed code to a dead address in production while working on a laptop. It takes nothing from the request but the slug (no `?to=`, ever: that would make it an open redirector), and `parseDestination()` runs on save and again on every scan. NO REDIRECT IN `next.config.ts` MAY MATCH `/q/<slug>`: Next matches redirects before rewrites and the filesystem, so one would silently take over and the console would do nothing. `/links` is the first-party page most codes land on, outside the `(public)` group because `PublicMain` renders at opacity 0 until hydration and this page has to read from the first HTML on fair-day signal. A sign-up made after a scan records the code in the subscription's existing `source` field (`qr:<slug>`, read from the address bar at submit, `src/lib/campaign/attribution.ts`). Scans are counted by a POST beacon: `ScanBeacon` in the root layout sees `?q=<slug>`, posts to `/api/q/[slug]/scan`, and marks the address `counted=1` so a reloaded tab is not counted twice. NEVER by a GET (link previews and mail scanners fetch a URL without running its page; `recordScan` is in the GET guard's `MUTATION_HELPERS`) and never from hosting request logs, which the privacy policy says are not used for analytics. No cookie, no browser storage, no IP or user agent stored, so the policy is unchanged. Reference: [docs/tracked-links.md](docs/tracked-links.md). A link to another site is a plain redirect and is not counted unless `countOffsite` sends it through a small counting page first, which is off by default and used by no printed code, because a script-driven forward is not always handed to the destination's app. What `/links` SAYS is edited at `/admin/links/page-content` (the three application buttons, which flip from "Opens soon" and the mailing list form to a real link, and the sections of rows): it can never render empty or broken, because `fetchLinksPage` serves the built-in rows in `src/content/links.ts` whenever the stored document is missing, damaged, all hidden or slow, and every stored address passes `parseDestination()` again on read (`tests/links-page-content.test.mjs`). `/admin/links` is also the dashboard: grouped by campaign, split by kind, scans beside sign-ups STARTED and CONFIRMED (a public sign-up is double opt-in, so one number would flatter the print run), counting distinct addresses and never rows, with the arithmetic in `src/lib/campaign/linkStats.ts`. Both of its reads are a range on one field so neither needs a declared index; keep them that way. Guards: `tests/link-stats.test.mjs`, `tests/tracked-links.test.mjs` (runs a table recorded from production through the shipping route: every printed code must answer with the identical status and `Location`), `tests/scan-counting.test.mjs`, `tests/campaign-attribution.test.mjs`.
...
- **The Firebase web API key is public by design, but its restrictions are not optional.** `NEXT_PUBLIC_FIREBASE_API_KEY` is inlined into the client bundle, so it ships to every browser and is safe in the open; Google's abuse scanner cannot tell it from a billable key and will mail about it if it appears in the repository. What actually matters is the key's API allowlist, which is the only thing bounding which of the project's enabled APIs a stranger can spend quota against. Both projects are restricted to the same five services (Identity Toolkit, Secure Token, Firestore, Storage, Firebase Installations) and carry **no** application restrictions: HTTP referrer restrictions on a Firebase browser key break sign-in, because the auth handler iframe does not reliably send a Referer header. `scripts/check-api-key-restrictions.mjs` guards all of this and runs in `e2e.yml`; run it by hand for prod (`--project naisi-website`). Dev sat fully unrestricted from 2026-05-25 to 2026-09-07 because a rollback that unblocked the sign-in rework was never re-corrected, which is the drift class the guard exists for.
